...
struct GPTBotCommand: AsyncParsableCommand {
    static var configuration: CommandConfiguration = CommandConfiguration(
        commandName: "gptbot",
        abstract: "A command line tool for interacting with OpenAI's chat models in a one-shot fashion."
    )
    ...
    @Flag(name: [.long], help: "If the specified output file already exists, overwrite it. Otherwise, append to it.")
    ...
    func validate() throws {
        ...
        for inputFile: String in inputFiles {
            let filePath: String =
                inputFile.hasPrefix("!") ? String(inputFile.dropFirst()) : inputFile
            if filePath != "stdin" {
